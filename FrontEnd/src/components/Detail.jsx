...
export const Detail = () => {
  ...
  const fetchAnime = async () => {
    ...
    setAnimedet(animedetail);
    // console.log(animedetail);
    setAnimegenres(animedetail.genres);
    setLoader(false);
  };
        ...
        loader ? <div className='h-screen w-full flex justify-center items-center'><span className="loader m-10"></span></div> :
          <>
            ...
            <div className='relative h-auto md:h-[82vh] flex justify-center'>
              ...
              <h1 className='text-white absolute bottom-0 p-10 text-2xl md:text-6xl font-bold text-center'>{animedet.original_title}</h1>
            ...
            {/* overview */}
            <h2 className='text-white text-center pt-5 px-3 md:px-60 font-Roboto text-[18px]'>{animedet.overview}</h2>

            <div className='text-blue-100 font-semibold my-3 flex justify-center'>
              <h2 className='bg-blue-600/30 border-2 border-blue-700 py-2 px-3 rounded-full'>Release Date : {animedet.release_date}</h2>
            </div>
